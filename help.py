...
def update_table_data_catalog(date_partition, storage_description, logger, glue_client_role, database_name_spec, table_name_spec, lake_control_account_id, s3_client):
    ...
    batch_spec = s3_location.split('/')[:3]

    limpa_particoes_existentes(database_name_spec, date_partition, glue_client_role, lake_control_account_id, table_name_spec)
    lista_num_lote = get_lista_num_lote_from_s3(batch_spec, f'{s3_prefix}' f'/num_ano_mes_dia=' f'{date_partition}', s3_client)
    ...
    try:
        ...
        for i in range(0, len(lista_num_lote), tamanho_lote):
            ...
            glue_client_role.batch_create_partition(DatabaseBame=database_name_spec, TableName=table_name_spec, CatalogId=lake_control_account_id, PartitionInputList=partition_inputs)
        ...
        if error.response['Error']['Code'] = 'ExpiredTokenException':
            logger.warning(f"Token de sessions AWS expirado, erro reproduzido ao tentar acessas {table_name_spec}")
...
def lista_objetos_s3(bucket_name, prefixo, s3_client, continuation_token=None):
    if continuation_token:
        return s3_client.lista_objects_v2(Bucket=bucket_name, Prefix=prefixo, Delimiter='/', continuationToken=continuation_token)
    else:
        return s3_client.lista_objects_v2(bucket=bucket_name, Prefix=prefixo, Delimiter='/')
...
def get_lista_num_lote_from_s3(bucket_name, prefixo, s3_client):
    ...
    while continua:
        resultado = lista_objetos_s3(bucket_name, prefixo, s3_client, continuation_token)

        if 'CommonPrefix' in resultado:
            for pasta in resultado['CommonPrefix']:
                num_lote = extrair_num_lote(pasat, exp_re)
                if num_lote:
                    pastas.add(num_lote)
...
def get_lista_particoes_spec_from_catalog(num_ano_mes_dia, glue_client_role, database_name_spec, table_name_spec, lake_control_account_id):
    ...
    while True:
        ...
        partition_info = glue_client_role.get_partitions(
                DatabaseNane=database_name_spec,
                TableName=table_name_spec,
                ExcludeColumnSchema=True,
...
def delete_batch_spec_partitions(lista_particoes, glue_client_role, lake_control_account_id, database_name_spec, table_name_spec):
    tamanho_lote = 30
    for i range(0, len(lista_particoes), tamanho_lote):
