...
def consulta_tabelas_main(logger: Any, glue_client: Any, spark: SparkSession, glue_context: GlueContext) -> Optional[DataFrame]:
    ...
    try:
        ...
        # Enhanced error analysis for entity-related issues
        error_message = str(e).lower()
        error_context = {"error": str(e)}
        
        if "entity" in error_message and "not found" in error_message:
            if "database" in error_message:
                error_context.update({
                    "entity_type": "database",
                    "issue": "Database does not exist in AWS Glue Catalog",
                    "suggestion": "Verify database name and ensure it exists in the Glue Data Catalog"
                })
            elif "table" in error_message:
                error_context.update({
                    "entity_type": "table", 
                    "issue": "Table does not exist in the specified database",
                    "suggestion": "Verify table name and ensure it exists in the database"
                })
            elif "partition" in error_message:
                error_context.update({
                    "entity_type": "partition",
                    "issue": "Partition does not exist for the table",
                    "suggestion": "Check if table has partitions or adjust partition filter logic"
                })
            else:
                error_context.update({
                    "entity_type": "unknown",
                    "issue": "Unknown entity not found",
                    "suggestion": "Check AWS Glue Catalog for missing entities"
                })
        elif "access" in error_message and "denied" in error_message:
            error_context.update({
                "entity_type": "permission",
                "issue": "Access denied to AWS Glue resources",
                "suggestion": "Verify IAM permissions for Glue operations"
            })
        
        logger.log_error_with_context(e, "table_query_unknown_error", **error_context)
...
def consulta_tabelas_mapped(
    ...
    if not table_mapping:
        logger.error(f"TABLE MAPPING NOT FOUND: No mapping configuration found for table key '{table_key}'. Available keys: {db_mapping.list_available_tables()}")
        return resultado_df
    
    full_table_name = f"{table_mapping.database}.{table_mapping.table}"
    logger.info(f"Querying mapped table: {full_table_name}")
    
    # Validate database and table existence before proceeding
    try:
        # Check if database exists
        glue_client.get_database(Name=table_mapping.database)
        logger.debug(f"DATABASE VALIDATED: Database '{table_mapping.database}' exists")
        
        # Check if table exists
        glue_client.get_table(DatabaseName=table_mapping.database, Name=table_mapping.table)
        logger.debug(f"TABLE VALIDATED: Table '{table_mapping.table}' exists in database '{table_mapping.database}'")
        
    except glue_client.exceptions.EntityNotFoundException as e:
        error_message = str(e)
        if "Database" in error_message:
            logger.error(f"DATABASE NOT FOUND: Database '{table_mapping.database}' does not exist in AWS Glue Catalog for table key '{table_key}'")
        elif "Table" in error_message:
            logger.error(f"TABLE NOT FOUND: Table '{table_mapping.table}' does not exist in database '{table_mapping.database}' for table key '{table_key}'")
        else:
            logger.error(f"ENTITY NOT FOUND: {error_message} for table key '{table_key}'")
        return resultado_df
    except Exception as e:
        logger.error(f"VALIDATION ERROR: Failed to validate entities for table key '{table_key}': {str(e)}")
        return resultado_df
    ...
    try:
        ...
        try:
            ...
            # Enhanced error logging to identify which entity is missing
            error_message = str(e)
            if "Database" in error_message or "database" in error_message:
                logger.error(f"DATABASE NOT FOUND: Database '{table_mapping.database}' does not exist in AWS Glue Catalog")
            elif "Table" in error_message or "table" in error_message:
                logger.error(f"TABLE NOT FOUND: Table '{table_mapping.table}' does not exist in database '{table_mapping.database}'")
            elif "Partition" in error_message or "partition" in error_message:
                logger.warning(f"PARTITION NOT FOUND: No partitions found for table '{table_mapping.database}.{table_mapping.table}' - proceeding without partition filter")
            else:
                logger.error(f"ENTITY NOT FOUND: Unknown entity missing for table '{table_mapping.database}.{table_mapping.table}': {error_message}")
            
            # Continue with query without partition filter
            if where_clause:
                query += f" WHERE {where_clause}"
