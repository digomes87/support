...
from pyspark.sql.functions import array, col, expr, lit, struct, udf, when
...
def formata_df(resultado_df: DataFrame, numero_lote_payload_dmp: int, logger: Logger) -> DataFrame:
...
def agregacoes_campo(resultado_df: DataFrame) -> DataFrame:
    agregacoes = {
        ...
        "dadosFatorRisco": F.struct(
            F.collect_set(
                F.col('Codigo_Exclusao_Comercial').alias("codigo")
            ).alias("listaExclusao"),
            F.collect_set(
                F.struct(
                    F.first('Apontamento_BX').alias("codigo")
                )
            ).alias("listaApontamentos"),
        ...
        "dadosPessoaFisica": F.struct(
            ...
            F.collect_set(F.struct('Vinculo').alias('Vinculo'))).alias('listaVinculos'),
            F.cillect_list(
                when(F.col('Cargos').isNull(),lit(-1)).otherwise(F.col('Cargos')).alias('cargo').alias('listaCargos')
            )
        ),
        ...
        "dadosVisaoCliente": F.struct(
                F.first('Target', True).alias('target').alias('rating')
        )
        ...
        "listaApontamentos": F.struct(
                F.when(F.col('Apontamento_BX').isNull(), -1).otherwise(F.col('Apontamento_BX').cast(IntegerType())).alias('codigo'),
                F.when(F.col('dias').isNull(), -1).otherwise(F.col('dias')).alias('recencia')
        ),
        ...
        "listaContratos": F.collect_list(
                F.struct(
                    F.first('valorTotalParcelaInterno').alias('valorCnotrato')
                )
        ),
...
def tratamento_df(resultado_df):
...
def calcula_idade(resultado_df):
...
def set_IndicadorSPI(resultado_df, Indicador_SPI):
    resultado_df = resultado_df.withColumn(
        'Indicador_SPI',
...
def set_IndicadorSPO(resultado_df, Indicador_SPO):
    resultado_df = resultado_df.withColumn(
        'Indicador_SPO',
...
def set_indicadorCNAO(resultado_df, Claro_Nao):
    resultado_df = resultado_df.withColumn(
        'Claro_Nao',
...
def compacta_em_lotes(resultado_df, numero_lote_payload_dmp):
